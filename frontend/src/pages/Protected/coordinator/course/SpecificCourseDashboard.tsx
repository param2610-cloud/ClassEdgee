...
import { departmentIdAtom } from "@/store/atom";
...
const SpecificCourseDashboard = () => {
  ...
  const [,setdepartmetntId] = useAtom(departmentIdAtom)
  ...
  useEffect(() => {
      if(course?.department_id){
        setdepartmetntId(course?.department_id.toString())
      }
  },[course])
  const fetchCourseSyllabi = async () => {
    try {
      const response = await fetch(
        `${domain}/api/v1/curriculum/syllabus/${id}`
      );
      const data = await response.json();
      setSyllabi(data);
    } catch (error) {
    ...
    <div className="container mx-auto p-6">
      ...
      <div className="grid gap-6">
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <span>Department: {course.departments?.department_name}</span>
              </div>
            </div>
            <div className="w-full flex justify-between">
              ...
              <div>
                <Button
                  onClick={() =>
                    navigate(
                      `/p/department/${course.department_id}/add-syllabus/${course.course_id}`
                    )
                  }
                >
                  Add Syllabus
        ...
        <Tabs defaultValue="syllabus" className="w-full">
          <TabsList>
            <TabsTrigger value="syllabus">Syllabus Structure</TabsTrigger>
            <TabsTrigger value="overview">Course Overview</TabsTrigger>
            <TabsTrigger value="sechdule">Class Sechdule</TabsTrigger>
          </TabsList>
            ...
            <div className="grid gap-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {syllabi.map((syllabus: SyllabusStructure) => (
                  ...
                  >
                    <CardHeader>
                      <CardTitle>Semester {syllabus.semester}</CardTitle>
                      <CardDescription>
                        {syllabus.subject_details.length || 0} Subjects
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                      <p className="text-sm text-gray-600">
                        Click to view or manage semester syllabus
                      </p>
                    </CardContent>
                  </Card>
                ))}
          ...
          <TabsContent value="overview">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Detailed information about the course structure and objectives
                </CardDescription>
              </CardHeader>
              <CardContent>
          ...
          <TabsContent value="sechdule">
                      {
                        syllabi.map((semester)=>(
                            <Card className="flex justify-between">
                                <CardHeader>
                                    Semester {semester.semester}
                                </CardHeader>
                                <CardFooter>
                                    <Button onClick={()=>navigate(`/p/course/${id}/semester/${semester.semester}/sechdule`)}>
                                    Sechdule
                                    </Button>
                                </CardFooter>
                            </Card>
